...
fviz_pca_biplot(compost_pca_est,
                ...
                palette = c("#010406FF", "#808080","#5C88DAFF" ),
                title = "PCA - Whole matrix geochemical characterization")  +
...
fig4a <- ggplot(data=si_ca, aes(x=si_ca_ratio, y=pb_ti_ratio, shape=Source, color=Source))
fig4a + geom_jitter()+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.title = element_text(size = 14), axis.text=element_text(size=12))+ 
  labs(x = "Si/Ca", y= "Pb/Ti") + scale_color_startrek()+ scale_x_continuous(limits=c(0,25), breaks = seq(0, 25, by = 5), expand=c(0, 0))+ scale_y_continuous(limits = c(0, 0.3))
...
cols2 <- c("#800026", "#E31A1C", "#FD8D3C", "#FEB24C", "#FED976")

brewer.pal(n = 9, name = "YlOrRd")

colnames(si_ca_city)[1] <- "Year Collected"

ggplot()+ geom_jitter(data=si_ca_city, aes(x=si_ca_ratio, y=pb_ti_ratio, color=`Year Collected`))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.title = element_text(size = 14), axis.text=element_text(size=12))+ 
  labs(x = "Si/Ca ", y= "Pb/Ti", fill="Year Collected")+scale_colour_gradientn(colours = cols2, 
                                                                               values = rescale(si_ca_city$year_collected, c(2006, 2008, 2009, 2010, 2011, 2021, 2022)),guide = "colorbar", limits=c(2006, 2022))+labs(fill='Year Collected')+ scale_x_continuous(limits=c(0,25), breaks = seq(0, 25, by = 5), expand=c(0, 0))+ scale_y_continuous(limits = c(0, 0.3))


#fig4c: non city 3 bins

si_ca_non_BINN <- non_city_raw %>% dplyr::select(year_collected, waste_stream_source, si_percent, ca_percent, pb_ug_g, ti_ug_g, loi)%>% mutate(si_ca_ratio=si_percent/ca_percent, pb_ti_ratio=pb_ug_g/ti_ug_g)%>%  mutate(Source=non_city_label) %>% na.omit


cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

fig4c <- ggplot(data=si_ca_non_BINN, aes(x=si_ca_ratio, y=pb_ti_ratio, color=waste_stream_source)) 

legend_title <- "Waste Stream Source" 

ggplot(data=si_ca_non_BINN, aes(x=si_ca_ratio, y=pb_ti_ratio, color=waste_stream_source))+ geom_jitter(shape=17)+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.title = element_text(size = 14), axis.text=element_text(size=12), aspect.ratio=0.75)+ labs(x = "Si/Ca", y= "Pb/Ti", color="Waste Stream Source")+ scale_y_continuous(limits = c(0, 0.3)) + scale_x_continuous(limits=c(0,25), breaks = seq(0, 25, by = 5), expand=c(0, 0))+ scale_color_manual(values=c( "#010406FF", "#808080","#5C88DAFF"))
